...
function treeToList (data, depth = 0, parent = null) {
    const  resultArray = [];

    data.forEach(el => {
        const row = { node: el, depth: depth + 1, parent };

        resultArray.push({ node: { name: el.name, rowId: el.rowId }, depth: depth + 1, parent });
        
        if (el.children && el.children.length) {
            resultArray.push(...treeToList(el.children, depth + 1, row));
        }
    });
